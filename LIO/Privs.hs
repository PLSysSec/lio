...
privilege allows you to behave as if @L_1 ``leq`` L_2@ even when that
is not the case.  The process of making data labeled @L_1@ affect data
labeled @L_2@ when @not (L_1 ``leq`` L_2)@ is called /downgrading/.

The basic method of the 'Priv' object is 'canFlowToP', which performs the
...
arguments depend on the particular label type.  (Of course, the symbol
'mintTCB' must not be available to untrusted code.)
...
class (Label l, PrivTCB p, Monoid p) => Priv l p where
    ...
    -- compare two labels in the presence of privileges.  If
    -- @'canFlowToP' p L_1 L_2@ holds, then privileges @p@ are sufficient
    -- to downgrade data from @L_1@ to @L_2@.  Note that @'canFlowTo' L_1
    -- L_2@ implies @'canFlowToP' p L_1 L_2@ for all @p@, but for some
    -- labels and privileges, 'canFlowToP' will hold even where
    -- 'canFlowTo' does not.
    ...
    --   2. @ L &#8849;&#8346; L_l@.
    --
    -- Operationally, @labelDiffP@ captures the minimum change required to
...
data NoPrivs = NoPrivs deriving (Show)
